...
class TresbolilloDialog(QDialog):
    ...
    def _number_records(self, records, direction):
        """Assign row and plant numbers according to the selected offset direction."""
        if not records:
            return []

        # records: point_id, grid_row, grid_column, x_coord, y_coord
        numbered_records = []

        if direction == "Y":
            # Vertical offset:
            # Rows (hileras) are vertical and numbered from west to east.
            # Plants are numbered inside each row from south to north.
            grouped = {}
            for point_id, grid_row, grid_column, x_coord, y_coord in records:
                grouped.setdefault(grid_column, []).append((point_id, grid_row, grid_column, x_coord, y_coord))

            ordered_groups = sorted(grouped.items(), key=lambda item: min(record[3] for record in item[1]))

            max_hilera = len(ordered_groups)
            max_planta = max(len(group_records) for _, group_records in ordered_groups)
            hilera_width = max(2, len(str(max_hilera)))
            planta_width = max(2, len(str(max_planta)))

            new_id = 1
            for hilera_number, (_, group_records) in enumerate(ordered_groups, start=1):
                ordered_points = sorted(group_records, key=lambda record: (record[4], record[3]))
                for planta_number, (_, grid_row, grid_column, x_coord, y_coord) in enumerate(ordered_points, start=1):
                    codigo = "H{}-P{}".format(
                        str(hilera_number).zfill(hilera_width),
                        str(planta_number).zfill(planta_width)
                    )
                    numbered_records.append((
                        new_id, grid_row, grid_column, hilera_number, planta_number, codigo, x_coord, y_coord
                    ))
                    new_id += 1

        else:
            # Horizontal offset:
            # Rows (hileras) are horizontal and numbered from south to north.
            # Plants are numbered inside each row from west to east.
            grouped = {}
            for point_id, grid_row, grid_column, x_coord, y_coord in records:
                grouped.setdefault(grid_row, []).append((point_id, grid_row, grid_column, x_coord, y_coord))

            ordered_groups = sorted(grouped.items(), key=lambda item: min(record[4] for record in item[1]))
            ...
            new_id = 1
            for hilera_number, (_, group_records) in enumerate(ordered_groups, start=1):
                ordered_points = sorted(group_records, key=lambda record: (record[3], record[4]))
                for planta_number, (_, grid_row, grid_column, x_coord, y_coord) in enumerate(ordered_points, start=1):
                    codigo = "H{}-P{}".format(
                        str(hilera_number).zfill(hilera_width),
                        str(planta_number).zfill(planta_width)
    ...
    def _write_output(self, output, crs, records):
        ...
        fields.append(QgsField("Planta", QVariant.Int))
        fields.append(QgsField("Codigo", QVariant.String, len=30))
        ...
        for point_id, row, column, hilera, planta, codigo, x_coord, y_coord in records:
            feature = QgsFeature(fields)
            feature.setGeometry(QgsGeometry.fromPointXY(QgsPointXY(x_coord, y_coord)))
            feature.setAttributes([point_id, hilera, planta, codigo, row, column])
    ...
    def run(self):
        try:
            ...
            numbered_records = self._number_records(records, direction)
            self._write_output(output, layer.crs(), numbered_records)
